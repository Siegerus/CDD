...
let arr = ['plum.png', 'orange.jpg', 'apple.bmp']; // так создаётся массив. элементам массива автоматически присваевается порядковый номер, умерация начинается с нуля.
...
let arr = ["first", 2, 3, "four", 5];                // так перебираем элементы массива
...
let ans = prompt("" , ""),
    arr = [];           // с помощтю split момещаем занные из переменной ans в массив arr
arr = ans.split(",");  // в скобках указываем, кка будут разделены элементы массива
console.log(arr);


let arr = ["qqw", "www", "1212" , 11];
i = arr.join(",") // join наоборот изз массива делает элемены одной строкой. в скобах тоже указываем как разделять
...
let arr = ["qqw", "www", "1212" , 11];
i = arr.sort(","); // sort сделает тоже мамое что и join, только ещё и отсортирует по алфави
