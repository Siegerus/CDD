...
const HAComponent = (): JSX.Element => {
	...
	return (
		<>
			<HelmetProvider>
				{' '}
				{/* Провайдер для установки тайтла в head для стр. Указываем в Helmet внутри каждого к-та */}
				<BrowserRouter
					future={{
					...
					<Routes>
						{' '}
						{/* Routes - комонент-кнтейнер для группы маршрутов и Route - сам маршрут*/}
						<Route path="/" element={<Layout />}>
							{' '}
							{/* path- путь в адресной строке. В нашем случае от корня */}
							<Route index element={<Main />} />{' '}
							{/* index - главный маршрут, который будет совпадать с "path='/'" */}
							<Route path="about/:year" element={<About />} />{' '}
							{/* Через ":" указывается параметр. Доступ к этим п-рам есть в "useParams" в к-те*/}
							<Route
								path="secret"
								element={
									<PrivateRoute>
										{' '}
										{/* В это к-те будет логика рендеринга. Либо ренд-г "children"(<Secret/>), либо переход на др. стр. В этой строке может показывать ошибку из за комментария*/}
										<Secret />
									</PrivateRoute>
								}
							/>
							...
							<Route
								path="*"
								element={
									/* элемент отрисуется, если ввести в адресной то, что не совпадёт ни с каким маршрутом (страница 404)*/
									<>
										<h1>Page 404</h1>
