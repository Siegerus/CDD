...
const UseEffectComponent = () => {
	...
	//   .then((json) => isNeedUpdate && setState(json))  // Проверка для того, что бы точно знать, что компонент перерисовался с актуальными данными
	...
	return (
		<>
			<div>
				{' '}
				{
					<p style={elementIsVisible ? { color: 'coral' } : undefined}>
						UseEffectComponent
					</p>
				}{' '}
			</div>
			<div>
				...
				<button type="button" onMouseDown={clickHandle}>
					inc +1
				</button>{' '}
				<p>{count}</p>
			</div>
