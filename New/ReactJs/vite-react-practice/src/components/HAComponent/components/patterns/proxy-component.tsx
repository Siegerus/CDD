...
const getComponentByType = (type, product) => {  // паттерн "контейнер" . Перебираем условия по типу и возвращаем нужный ком-т
	switch (type) {
		case Type.SALE:
...
const ProxyComponent = () => { // Это не прокси компонент. Просто название к-та
	return (
		<ul>
			{products.map((product, i) => {
				return (
					<li key={i}>
						...
						{getComponentByType(product.type, product)} {/* Используем контейнер(добавляем нужный ком-т взависимости от типа) */}
					</li>
				)
			})}
		</ul>
...
const NewProduct = (props) => {
	...
	return (
		<Product className={`product_new ${className}`} {...restProps} />  /* в нём создаём св-во с конкретным классом  и прокидываем пропсы от родителя просто как есть */
...
const SaleProduct = (props) => {
	...
	return (
		<Product className={`product_sale ${className}`} {...restProps} />  
	)
}
