...
const Form = () => {
	...
	const changeInputHandle = (e: ChangeEvent<HTMLInputElement>) => {
		//типы для событий импортируем из React
		setFormData({
			...formData,
			[e.target.name]: e.target.value, // Копируем всё что есть в объекте "formData" через "..." и меняем нужное св-во
		});
	};
	...
	return (
		<>
			...
			>
				...
				>
					{isVisibleDetails ? ' Спрятать' : 'Показать'}{' '}
				</button>
			</form>
