...
// Перебор объекта
Object.defineProperty(ob, "width", {
  /* способ менять зачение в объекте */ value: 400,
});

console.log(ob);

Object.keys(ob).forEach((key) => {
  /* ".keys(ob)"-метод, который вернёт из объкта его ключи */
...
Object.entries(ob).forEach(([key, value]) => {
  /*"entries"-метод, который вернёт из объкута пару ключ/значение в виде массивов */
...
let entries =
  Object.entries(
    ob
  ); /* в переменной массив, который содежит массивы из пар ключ/значение */

entries.splice(1, 1, [
  "min-height",
  300,
]); /* с помощью "splice" можем поменять нужные ключ/значение массива */
...
entries = entries.map((item) => {
  return (item = [
    "min-height",
    300,
  ]); /*а с помощью "map" сразу все элементы массива (ключ/значение). "return" обязателен */
});
...
let blobForLink = new Blob(["./../img/icon_card-heart.svg"], {
  type: "image/png",
});
...
mySelect.addEventListener(
  "input",
  () => (document.cookie = `cityValue=${mySelect.value}; max-age=10`)
);
...
for (let key in storagee) {
  // или можно перебирать через Object.entries / Object.key
...
openRequest.addEventListener("upgradeneeded", () => {
  //Обновление бд. Событие также работает, если базы ещё не существует
  ...
  if (!db.objectStoreNames.contains("books")) {
    // если хранилище "objectStore" не существует
...
openRequest.addEventListener("success", (e) => {
  // После upgradeneeded сработает событие success
...
openRequestt.onsuccess = () => {
  ...
  cursorRequest.onsuccess = () => {
    if (cursorRequest.result) {
      ...
      cursorRequest.result.continue(); /* продвинуть курсор к следующему значению */ /* продвинуть курсор на count позиций, пропустив значения */
      /* cursorRequest.result.advance(3); */
...
function zoomIn(el) {
  ...
  if (el.classList.contains("zoomed"))
    el.ontransitionend = (e) => onTransitionEnd(e);
}
function onZoom() {
  zoomIn(plane);
}
...
document.addEventListener("keydown", (e) => {
  if (e.key == "q")
    showCircle(150, 150, 200, function (div, lh) {
...
let targetEll = document.querySelector("body > div.any-section__frop-ball");
targetEll.addEventListener("click", (e) => {
  ...
  animate({
    ...
    draw: function (progress) {
      targetEll.style.left = progress * 1000 + "px";
    },
  });
});
