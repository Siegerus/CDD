...
let resultt = Object.entries(userssss).map(([key, value]) => [
  key + " changedKey",
  value + " changedValue",
]);
...
let getTopSalary = (obj) => {
  ...
  allValues.sort(
    ([currKey, currValue], [nextKey, nextValue]) => currValue - nextValue
  ); // 2
...
let toRandomize = (arr) => {
  let res = arr.map(
    (item, i, arr) => arr[Math.floor(Math.random(i) * arr.length)]
  );
...
let arrrrrrr = [
  1,
  2,
  [true, [4, [54, true, "111", { key: "value" }], 6], false],
  ["smth", 34],
];
let newArr = [];
...
// ф-ция каждую итерацию заменяет соответствующий элемент массива на "test"
function testFn() {
  debugger;
  ...
  for (let i = 0; i < testArr.length; i++) {
    targetArr = [...testArr.slice(0, i), "test", ...testArr.slice(i + 1)];
  }
  return targetArr;
}
